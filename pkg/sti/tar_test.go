...
func getRequestHandler() *requestHandler {
	request := &STIRequest{}
	return &requestHandler{request: request, untarTimeout: 2 * time.Minute}
}
...
func TestCreateTarUpload(t *testing.T) {
	rh := getRequestHandler()
	var err error
	rh.request.workingDir, err = ioutil.TempDir("", "testtar")
	defer os.RemoveAll(rh.request.workingDir)
	if err != nil {
		t.Fatalf("Cannot create temp directory for test: %v", err)
	}

	uploadDir := filepath.Join(rh.request.workingDir, "upload")
	err = os.Mkdir(uploadDir, 0700)
	if err != nil {
		t.Fatalf("Cannot create upload directory for test: %v", err)
	}
	...
	err = createTestFiles(uploadDir, testFiles)
	if err != nil {
		t.Fatalf("Cannot create test files: %v", err)
	}

	tarFile, err := rh.createTarUpload()
	if err != nil {
...
func TestExtractTarStream(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		rh.extractTarStream(destDir, reader)
	}()
	wg.Wait()
...
func TestExtractTarStreamTimeout(t *testing.T) {
	...
	wg.Add(2)
	rh := getRequestHandler()
	rh.untarTimeout = 100 * time.Millisecond
	go func() {
		defer wg.Done()
	...
	go func() {
		defer wg.Done()
		extractError <- rh.extractTarStream(destDir, reader)
	}()
	wg.Wait()
	err = <-extractError
